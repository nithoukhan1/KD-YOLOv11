...
class FeatureDistillLoss(nn.Module):
    def __init__(self, teacher_weights, base_criterion, student_model, alpha=0.5):
        ...
        # Hook to capture student feature maps before the detection head
        def hook_fn(module, input):
            # PyTorch hooks receive a tuple; index 0 extracts the list of feature maps
            self.student_features = input 
            
        student_model.model[-1].register_forward_pre_hook(hook_fn)

    def forward(self, preds, batch):
        # Calculate standard YOLO loss
        loss, loss_items = self.base_criterion(preds, batch)
        imgs = batch["img"]
        
        # Ensure teacher is on the correct GPU and matches image precision
        ...
        # Calculate KD Loss across multi-scale features
...
class KDDetectionTrainer(DetectionTrainer):
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        # Safely extract custom argument to bypass YOLO's strict validation
