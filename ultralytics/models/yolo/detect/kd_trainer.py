...
# A wrapper to inject distillation into the model's forward pass
class DistillLossWrapper(nn.Module):
    ...
    def forward(self, preds, batch):
        ...
        # 3. Calculate KD Loss (MSE alignment between Student/Teacher logits)
        kd_loss = 0.0
        ...
        # 5. Append KD to the logging tensor so it reaches the CSV/Console
...
class KDDetectionTrainer(DetectionTrainer):
    def __init__(self, cfg=None, overrides=None, _callbacks=None):
        ...
        # Force the names into the aggregator
        self.loss_names = self.custom_loss_names
        # Resize the average loss tracker to ensure Rank 0 records the 4th value
        ...
        if teacher_path:
            ...
            # This is the only way to ensure distillation runs in the training loop
            self.model.criterion = DistillLossWrapper(teacher_model, self.model.criterion, kd_val)
